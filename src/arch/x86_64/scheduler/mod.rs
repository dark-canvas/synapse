...
pub fn yield_task() -> ! {
    unsafe {
        // TODO: for a short period of time (because rip is in the register snapshot, but not queried)
        // rip == 0x0 until the intrinsics::return_address() is called.
        (*CURRENT_TASK).registers = get_register_snapshot!();
        (*CURRENT_TASK).registers.rip = intrinsics::return_address() as u64;
        //(*CURRENT_TASK).registers.rflags = rflags::read_raw();

        // TODO: need to save these...
        let current_cs = CS::get_reg();
        let current_ss = SS::get_reg();

        // get next... (for now, re-use currnt task...
        let next_task = &*CURRENT_TASK;
        let mut next_stack_frame = InterruptStackFrame::new(
            VirtAddr::new( next_task.registers.rip),
            current_cs,
            RFlags::from_bits_retain(next_task.registers.rflags),
            VirtAddr::new(next_task.registers.rsp),
            current_ss,
        );
